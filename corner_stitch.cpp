	...
	if(solid_area <= threshold * (right - left) * (top - bottom)){
		...
		for(int i = 0; i < cost_tile.size(); i++){
			...
			if(TiGetBody(BL(cost_tile[i])) == SOLID_TILE && TiGetBody(TR(cost_tile[i])) == SOLID_TILE &&
				...
				if(macro[TiGetClient(BL(cost_tile[i]))]->name() != "null" && macro[TiGetClient(TR(cost_tile[i]))]->name() != "null"){
					bool pre = false;
			...
			if(TiGetBody(LB(cost_tile[i])) == SOLID_TILE && TiGetBody(RT(cost_tile[i])) == SOLID_TILE &&
				...
				// Do not space e(macro, null)
				if(macro[TiGetClient(LB(cost_tile[i]))]->name() != "null" && macro[TiGetClient(RT(cost_tile[i]))]->name() != "null"){
					bool pre = false;
	...
	while(Gv.longest_path(false) > chip_height){
		...
		for(int i = 0; i < v_zero_slack.size(); i++){
			if(v_zero_slack[i]->from == 0){
				cout << "source to " << macro[v_zero_slack[i]->to - 1]->name() << endl;
			}
			else if(v_zero_slack[i]->to == macro.size() + 1){
				cout << macro[v_zero_slack[i]->from - 1]->name() << " to sink" << endl;
			}
			else{
				cout << macro[v_zero_slack[i]->from - 1]->name() << " to " << macro[v_zero_slack[i]->to - 1]->name() << endl;
			}
		}
		for(int i = 0; i < v_zero_slack.size(); i++){
